Shader "gs/gsUInt_Tutorial"
{
  ...
  RWStructuredBuffer<GUInt_Tutorial> gUInt_Tutorial;
  ...
  uint3 onRenderObject_LIN(uint i) { uint3 LIN = u000; uint index = 0; onRenderObject_LIN(g.displaySpheres, g.sphereN, i, index, LIN); onRenderObject_LIN(g.ADraw_textN, i, index, LIN); onRenderObject_LIN(g.ADraw_boxEdgeN, i, index, LIN); return LIN; }
  ...
  {
    float r = 0.2f;
    return vert_ADraw_Sphere(i * r * 2 * f010, r, palette(i / (g.sphereN - 1.0f)), i, j, o);
  }
