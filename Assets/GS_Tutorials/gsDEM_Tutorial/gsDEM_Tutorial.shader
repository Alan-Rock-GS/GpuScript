Shader "gs/gsDEM_Tutorial"
{
  ...
  uint3 onRenderObject_LIN(uint i) { uint3 LIN = u000; uint index = 0; onRenderObject_LIN(g.displayNodes, g.nodeN, i, index, LIN); onRenderObject_LIN(g.ADraw_textN, i, index, LIN); onRenderObject_LIN(g.ADraw_boxEdgeN, i, index, LIN); return LIN; }
  ...
  {
    Node n = nodes[i];
    float4 c = i == 0 ? YELLOW : i == g.nodeN - 1 ? RED : GREEN;
    return vert_ADraw_Sphere(n.position, g.resolution / 2, c, i, j, o);
  }
