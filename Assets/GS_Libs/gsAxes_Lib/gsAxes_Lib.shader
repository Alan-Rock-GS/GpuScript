Shader "gs/gsAxes_Lib"
{
  ...
  #define g gAxes_Lib[0]
  struct GAxes_Lib
  {
    uint BDraw_ABuff_IndexN, BDraw_ABuff_BitN, BDraw_ABuff_N, BDraw_ABuff_BitN1, BDraw_ABuff_BitN2, BDraw_omitText, BDraw_includeUnicode, BDraw_fontInfoN, BDraw_textN, BDraw_textCharN, BDraw_boxEdgeN, drawGrid, drawBox, drawAxes, customAxesRangeN, zeroOrigin, buildText, showAxes, showOutline, showNormalizedAxes;
    float BDraw_fontSize, BDraw_boxThickness, boxLineThickness, axesOpacity;
    float4 BDraw_boxColor;
    float2 GridX, GridY, GridZ, textSize;
    float3 axesRangeMin, axesRangeMax, axesRangeMin1, axesRangeMax1, axesRangeMin2, axesRangeMax2, axesColor;
  };
  ...
  RWStructuredBuffer<GAxes_Lib> gAxes_Lib;
  ...
  uint3 onRenderObject_LIN(uint i) { uint3 LIN = u000; uint index = 0; onRenderObject_LIN(g.drawBox && g.drawAxes, g.BDraw_textN, i, index, LIN); onRenderObject_LIN(g.drawBox, 12, i, index, LIN); return LIN; }
  ...
  v2f vert_BDraw_Box(uint i, uint j, v2f o) { return vert_BDraw_BoxFrame(gridMin(), gridMax(), g.boxLineThickness, DARK_BLUE, i, j, o); }
