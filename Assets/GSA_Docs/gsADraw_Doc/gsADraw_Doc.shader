Shader "gs/gsADraw_Doc"
{
  ...
  struct GADraw_Doc
  {
    float rotation_time, ADraw_fontSize, ADraw_boxThickness;
    uint shapeN, speed, uintN, ADraw_omitText, ADraw_includeUnicode, ADraw_fontInfoN, ADraw_textN, ADraw_textCharN, ADraw_boxEdgeN, ADraw_AAppendBuff_IndexN, ADraw_AAppendBuff_BitN, ADraw_AAppendBuff_N, ADraw_AAppendBuff_BitRowN, ADraw_AAppendBuff_BitColN;
  ...
  RWStructuredBuffer<GADraw_Doc> gADraw_Doc;
  RWStructuredBuffer<uint> ADraw_tab_delimeted_text, uints, ADraw_AAppendBuff_Bits, ADraw_AAppendBuff_Sums, ADraw_AAppendBuff_Indexes, ADraw_AAppendBuff_ColN_Sums;
  ...
  uint3 onRenderObject_LIN(uint i) { uint3 LIN = u000; uint index = 0; onRenderObject_LIN(g.shapeN * g.shapeN * g.shapeN, i, index, LIN); onRenderObject_LIN(g.ADraw_textN, i, index, LIN); onRenderObject_LIN(g.ADraw_boxEdgeN, i, index, LIN); return LIN; }
  ...
  v2f vert_ADraw_Sphere(float3 p, float r, float4 color, uint i, uint j, v2f o) { float4 p4 = float4(p, 1), quadPoint = ADraw_Sphere_quadPoint(r, j); o.pos = mul(UNITY_MATRIX_P, mul(UNITY_MATRIX_V, p4) + quadPoint); o.wPos = p; o.uv = quadPoint.xy / r; o.normal = -f001; o.color = color; o.ti = float4(i, 0, ADraw_Draw_Sphere, 0); return o; }
  ...
  v2f vert_Draw_Shapes(uint i, uint j, v2f o)
  {
    uint3 id = i_to_id(i, u111 * g.shapeN);
    float3 p = lerp(f___, f111, id / (g.shapeN - 1.0f));
    ...
    if (underGround) return vert_ADraw_Sphere(p, 0.8f / g.shapeN * 2, color, i, j, o);
    if (!inVortex) return o;
    float v = lerp(1, 0.1f, len / vortexR), r = v * 0.01f;
    p = rotateY(p, g.rotation_time * v);
    float3 dp = v * cross(normalize(p), f010) / 2;
    return vert_ADraw_Arrow(p - dp, p + dp, r, color, i, j, o);
  }
  ...
  v2f vert_GS(uint i, uint j, v2f o)
  {
    ...
    if (level == ++index) { o = vert_Draw_Shapes(i, j, o); o.tj.x = 0; }
