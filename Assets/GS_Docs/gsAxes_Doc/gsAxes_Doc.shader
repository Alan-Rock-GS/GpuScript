Shader "gs/gsAxes_Doc"
{
  ...
  uint3 onRenderObject_LIN(uint i) { uint3 LIN = u000; uint index = 0; onRenderObject_LIN(g.Axes_Lib_drawBox && g.Axes_Lib_drawAxes, g.Axes_Lib_BDraw_textN, i, index, LIN); onRenderObject_LIN(g.Axes_Lib_drawBox, 12, i, index, LIN); return LIN; }
  ...
  uint Axes_Lib_BDraw_SignalSmpN(uint chI) { return 1024; }
  float Axes_Lib_BDraw_SignalThickness(uint chI, uint smpI) { return 0.004f; }
  float Axes_Lib_BDraw_SignalSmpV(uint chI, uint smpI) { return 0; }
  float4 Axes_Lib_BDraw_SignalColor(uint chI, uint smpI) { return YELLOW; }
  ...
  float4 Axes_Lib_BDraw_SignalBackColor(uint chI, uint smpI) { return float4(1, 1, 1, 0.2f); }
  ...
  v2f vert_GS(uint i, uint j, v2f o)
  {
    ...
    if (level == ++index) { o = vert_Axes_Lib_BDraw_Text(i, j, o); o.tj.x = 0; }
    else if (level == ++index) { o = vert_Axes_Lib_BDraw_Box(i, j, o); o.tj.x = 0; }
    return o;
  }
