Shader "gs/gsBDraw_Doc"
{
  ...
  uint3 onRenderObject_LIN(uint i) { uint3 LIN = u000; uint index = 0; onRenderObject_LIN(g.shapeN * g.shapeN * g.shapeN, i, index, LIN); onRenderObject_LIN(g.BDraw_textN, i, index, LIN); onRenderObject_LIN(g.BDraw_boxEdgeN, i, index, LIN); return LIN; }
  ...
  v2f o_wPos(float3 wPos, v2f o) { o.wPos = wPos; return o; }
  float BDraw_wrapJ(uint j, uint n) { return ((j + n) % 6) / 3; }
  v2f o_r(float r, v2f o) { o.ti.w = r; return o; }
  float o_r(v2f o) { return o.ti.w; }
  float4 frag_BDraw_Line(v2f i) { float3 p0 = i.p0, p1 = i.p1; float lineRadius = o_r(i); float2 uv = i.uv; float r = dot(uv, uv), r2 = lineRadius * lineRadius; float4 color = i.color; float3 p10 = p1 - p0; float lp10 = length(p10); if (uv.x < 0) r /= r2; else if (uv.x > lp10) { uv.x -= lp10; r = dot(uv, uv) / r2; } else { uv.x = 0; r = dot(uv, uv) / r2; } if (r > 1.0f || color.a == 0) return f0000; float3 n = new float3(uv, r - 1), _LightDir = new float3(0.321f, 0.766f, -0.557f); float lightAmp = max(0.0f, dot(n, _LightDir)); float4 diffuse_Light = (lightAmp + UNITY_LIGHTMODEL_AMBIENT) * color; float spec = max(0, (lightAmp - 0.95f) / 0.05f); color = lerp(diffuse_Light, f1111, spec / 4); color.a = 1; return color; }
  float4 frag_BDraw_Arrow(v2f i) { float3 p0 = i.p0, p1 = i.p1; float lineRadius = o_r(i); float2 uv = i.uv; float r = dot(uv, uv), r2 = lineRadius * lineRadius; float4 color = i.color; float3 p10 = p1 - p0; float lp10 = length(p10); if (uv.x < 0) r /= r2; else if (uv.x > lp10 - lineRadius * 3 && abs(uv.y) > lineRadius) { uv.x -= lp10; uv = rotate_sc(uv, -sign(uv.y) * 0.5f, 0.866025404f); uv.x = 0; r = dot(uv, uv) / r2; } else if (uv.x > lp10) { uv.x -= lp10; r = dot(uv, uv) / r2; } else { uv.x = 0; r = dot(uv, uv) / r2; } if (r > 1.0f || color.a == 0) return f0000; float3 n = new float3(uv, r - 1), _LightDir = new float3(0.321f, 0.766f, -0.557f); float lightAmp = max(0.0f, dot(n, _LightDir)); float4 diffuse_Light = (lightAmp + UNITY_LIGHTMODEL_AMBIENT) * color; float spec = max(0, (lightAmp - 0.95f) / 0.05f); color = lerp(diffuse_Light, f1111, spec / 4); color.a = 1; return color; }
  float4 frag_BDraw_LineSegment(v2f i) { float3 p0 = i.p0, p1 = i.p1; float lineRadius = o_r(i); float2 uv = i.uv; float r = dot(uv, uv), r2 = lineRadius * lineRadius; float4 color = i.color; float3 p10 = p1 - p0; float lp10 = length(p10); uv.x = 0; r = dot(uv, uv) / r2; if (r > 1.0f || color.a == 0) return f0000; float3 n = new float3(uv, r - 1), _LightDir = new float3(0.321f, 0.766f, -0.557f); float lightAmp = max(0.0f, dot(n, _LightDir)); float4 diffuse_Light = (lightAmp + UNITY_LIGHTMODEL_AMBIENT) * color; float spec = max(0, (lightAmp - 0.95f) / 0.05f); color = lerp(diffuse_Light, f1111, spec / 4); color.a = 1; return color; }
  ...
  {
    uint chI = o_i(i), SmpN = BDraw_SignalSmpN(chI);
  ...
  float4 frag_BDraw_GS(v2f i, float4 color)
  {
    switch (o_drawType(i))
    {
      ...
      case BDraw_Draw_Text3D:
        BDraw_TextInfo t = BDraw_textInfo(o_i(i));
  ...
  v2f vert_BDraw_Sphere(float3 p, float r, float4 color, uint i, uint j, v2f o) { float4 q = BDraw_Sphere_quadPoint(r, j); return o_i(i, o_drawType(BDraw_Draw_Sphere, o_color(color, o_normal(-f001, o_uv(q.xy / r, o_pos_PV(p, q, o_wPos(p, o))))))); }
  ...
  v2f vert_BDraw_LineArrow(float dpf, float3 p0, float3 p1, float r, float4 color, uint i, uint j, v2f o) { return o_i(i, o_p0(p0, o_p1(p1, o_r(r, o_drawType(dpf == 1 ? BDraw_Draw_Line : BDraw_Draw_Arrow, o_color(color, o_uv(BDraw_LineArrow_uv(dpf, p0, p1, r, j), o_pos_c(BDraw_LineArrow_p4(dpf, p0, p1, r, j), o)))))))); }
  ...
  v2f vert_Draw_Shapes(uint i, uint j, v2f o)
  {
    ...
    if (underGround) return vert_BDraw_Sphere(p, 0.8f / g.shapeN * 2, color, i, j, o);
  ...
  v2f vert_BDraw_Text(BDraw_TextInfo t, uint i, uint j, v2f o)
  {
    ...
    if (quadType == BDraw_Text_QuadType_Billboard)
    {
      ...
      o = o_i(i, o_drawType(BDraw_Draw_Text3D, o_r(i, o_color(color, o_pos_PV(p, billboardQuad + float4(jp, 0), o_normal(f00_, o_uv(float2(BDraw_wrapJ(j, 2), BDraw_wrapJ(j, 4)) * uvSize, o)))))));
    }
    else if (quadType == BDraw_Text_QuadType_Arrow)
    {
      ...
      o = o_uv(dot(p - _WorldSpaceCameraPos, cross(right, up)) < 0 ? float2(length(q1 - q0) / h * BDraw_wrapJ(j, 1), BDraw_wrapJ(j, 2) - 0.5f) : float2(length(q1 - q0) / h * (1 - BDraw_wrapJ(j, 1)), 0.5f - BDraw_wrapJ(j, 2)), o_drawType(BDraw_Draw_Text3D, vert_BDraw_Arrow(q0, q1, h * 0.165f, color, i, j, o)));
    }
    else if (quadType == BDraw_Text_QuadType_FrontBack || quadType == BDraw_Text_QuadType_Switch || dot(p - _WorldSpaceCameraPos, cross(right, up)) > 0)
    {
      ...
      if (draw)
      {
        ...
        o = o_i(i, o_drawType(BDraw_Draw_Text3D, o_r(i, o_color(color, o_pos_c(o.wPos = j % 5 == 0 ? p3 : j == 1 ? p2 : j == 4 ? q0 : q1, o_normal(cross(right, up), o_uv(quadType == BDraw_Text_QuadType_Switch && dot(p - _WorldSpaceCameraPos, cross(right, up)) < 0 ? float2(1 - BDraw_wrapJ(j, 2), BDraw_wrapJ(j, 4)) * uvSize : float2(BDraw_wrapJ(j, 2), BDraw_wrapJ(j, 4)) * uvSize, o)))))));
      }
    }
    return o;
  }
  v2f vert_BDraw_Text(uint i, uint j, v2f o) { return vert_BDraw_Text(BDraw_textInfo(i), i, j, o); }
  v2f vert_BDraw_Line(float3 p0, float3 p1, float r, float4 color, uint i, uint j, v2f o) { return o_i(i, o_p0(p0, o_p1(p1, o_r(r, o_drawType(BDraw_Draw_Line, o_color(color, o_uv(BDraw_Line_uv(p0, p1, r, j), o_pos_c(BDraw_LineArrow_p4(1, p0, p1, r, j), o)))))))); }
  ...
  v2f vert_GS(uint i, uint j, v2f o)
  {
    ...
    if (level == ++index) o = vert_Draw_Shapes(i, j, o);
