Shader "gs/gsRand_Doc"
{
  ...
  uint3 onRenderObject_LIN(uint i) { uint3 LIN = u000; uint index = 0; onRenderObject_LIN(g.drawGroup == DrawGroup_Average, 1, i, index, LIN); onRenderObject_LIN(g.drawGroup == DrawGroup_Average, 1, i, index, LIN); onRenderObject_LIN(g.drawGroup == DrawGroup_Average, 1, i, index, LIN); onRenderObject_LIN(g.drawGroup == DrawGroup_Anneal, g.starN, i, index, LIN); onRenderObject_LIN(g.drawGroup == DrawGroup_Anneal, g.starN, i, index, LIN); onRenderObject_LIN(g.drawGroup == DrawGroup_Anneal, 13, i, index, LIN); onRenderObject_LIN(g.BDraw_textN, i, index, LIN); onRenderObject_LIN(g.BDraw_boxEdgeN, i, index, LIN); return LIN; }
  ...
  uint BDraw_SampleN() { return g.pntN; }
  ...
  {
    float3 p0 = cities[bestPath(i)], p1 = cities[bestPath((i + 1) % g.starN)];
  ...
  v2f vert_Draw_Cities(uint i, uint j, v2f o) { return vert_BDraw_Sphere(cities[i], 0.02f, YELLOW, i, j, o); }
  ...
  v2f vert_GS(uint i, uint j, v2f o)
  {
    ...
    else if (level == ++index) { o = vert_Draw_Star_Path(i, j, o); o.tj.x = 0; }
  ...
  float BDraw_SampleV(uint i) { return lerp(-1, 1, Rand_rFloat(i)); }
