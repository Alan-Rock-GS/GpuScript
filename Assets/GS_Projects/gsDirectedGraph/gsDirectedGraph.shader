Shader "gs/gsDirectedGraph"
{
  ...
  float radius() { return 0.025f * g.plotScale; }
  ...
  uint3 onRenderObject_LIN(uint i) { uint3 LIN = u000; uint index = 0; onRenderObject_LIN(g.ADraw_textN, i, index, LIN); onRenderObject_LIN(g.ADraw_boxEdgeN, i, index, LIN); onRenderObject_LIN(g.nodeN, i, index, LIN); onRenderObject_LIN(g.edgeN, i, index, LIN); return LIN; }
  ...
  v2f vert_drawNodes(uint i, uint j, v2f o)
	{
		float color = 0;
		for (int I = 0; I < g.stageN; I++) if (i == bestPath[I]) { color = 0.5f; break; }
		if (i == 0) color = 0.5f;
		return vert_ADraw_Sphere(nodePs[i], radius(), palette(color), i, j, o);
	}
  ...
  v2f vert_drawEdges(uint i, uint j, v2f o)
	{
		float r = radius();
		Edge e = edges[i];
		...
		r /= 10;
		bool found = false;
		for (int I = 1; I < g.stageN; I++) if (e.node0 == bestPath[I - 1] && e.node1 == bestPath[I]) { r *= 3; found = true; break; }
		if (!found && e.stage == 0 && e.node1 == bestPath[1]) r *= 3;
		return vert_ADraw_Arrow(p0 + d, p1 - d, r, palette(e.cost / 9), i, j, o);
	}
