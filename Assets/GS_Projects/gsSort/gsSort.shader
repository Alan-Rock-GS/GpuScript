Shader "gs/gsSort"
{
  ...
  struct GSort
  {
    uint useUpperTriangular, arrayLength, numberOfArrays, runtimeN, benchmarkType, vsN, BDraw_ABuff_IndexN, BDraw_ABuff_BitN, BDraw_ABuff_N, BDraw_ABuff_BitN1, BDraw_ABuff_BitN2, BDraw_omitText, BDraw_includeUnicode, BDraw_fontInfoN, BDraw_textN, BDraw_textCharN, BDraw_boxEdgeN, Rand_N, Rand_I, Rand_J;
  ...
  v2f vert_vs_arrows(uint i, uint j, v2f o) { float r = get_r(); return vert_BDraw_Arrow(get_p0(i) - r * f010, get_p1(i) + r * f010, r / 4, palette(vs[i]), i, j, o); }
  ...
  v2f vert_GS(uint i, uint j, v2f o)
  {
    ...
    if (level == ++index) o = vert_vs0(i, j, o);
    else if (level == ++index) o = vert_vs1(i, j, o);
    else if (level == ++index) o = vert_vs_arrows(i, j, o);
