Shader "gs/gsSort"
{
  ...
  uint3 onRenderObject_LIN(uint i) { uint3 LIN = u000; uint index = 0; onRenderObject_LIN(g.node_size > 1, g.vsN, i, index, LIN); onRenderObject_LIN(g.node_size > 1, g.vsN, i, index, LIN); onRenderObject_LIN(g.node_size > 1, g.vsN, i, index, LIN); onRenderObject_LIN(g.ADraw_textN, i, index, LIN); onRenderObject_LIN(g.ADraw_boxEdgeN, i, index, LIN); return LIN; }
  ...
  v2f vert_vs0(uint i, uint j, v2f o) { return vert_ADraw_Sphere(get_p0(i), get_r(), palette(vs[i]), i, j, o); }
  v2f vert_vs1(uint i, uint j, v2f o) { return vert_ADraw_Sphere(get_p1(i), get_r(), palette(vs[i]), i, j, o); }
  ...
  v2f vert_vs_arrows(uint i, uint j, v2f o) { float r = get_r(); return vert_ADraw_Arrow(get_p0(i) - r * f010, get_p1(i) + r * f010, r / 4, palette(vs[i]), i, j, o); }
  ...
  v2f vert_GS(uint i, uint j, v2f o)
  {
    ...
    else if (level == ++index) { o = vert_ADraw_Text(i, j, o); o.tj.x = 3; }
    else if (level == ++index) { o = vert_ADraw_Box(i, j, o); o.tj.x = 3; }
    return o;
  }
