Shader "gs/gsSwarm"
{
  ...
  uint3 onRenderObject_LIN(uint i) { uint3 LIN = u000; uint index = 0; onRenderObject_LIN(g.particleShape == ParticleShape_Sphere, g.particleN, i, index, LIN); onRenderObject_LIN(g.particleShape == ParticleShape_Line, g.particleN, i, index, LIN); onRenderObject_LIN(g.particleShape == ParticleShape_Arrow, g.particleN, i, index, LIN); onRenderObject_LIN(g.ADraw_boxEdgeN, i, index, LIN); onRenderObject_LIN(g.particleShape == ParticleShape_Text || g.particleShape == ParticleShape_Numbers || g.particleShape == ParticleShape_Letters, g.particleN, i, index, LIN); onRenderObject_LIN(g.particleShape == ParticleShape_Point, g.particleN, i, index, LIN); return LIN; }
  ...
  uint ADraw_o_drawType(v2f o) { return roundu(o.ti.z); }
  float4 frag_ADraw_Sphere(v2f i) { float2 uv = i.uv; float r = dot(uv, uv); float4 color = i.color; if (r > 1.0f || color.a == 0) return f0000; float3 n = new float3(uv, r - 1), _LightDir = new float3(0.321f, 0.766f, -0.557f); float lightAmp = max(0.0f, dot(n, _LightDir)); float4 diffuse_Light = (lightAmp + UNITY_LIGHTMODEL_AMBIENT) * color; float spec = max(0, (lightAmp - 0.95f) / 0.05f); color = lerp(diffuse_Light, f1111, spec / 4); color.a = 1; return color; }
  float4 frag_ADraw_Mesh(v2f i) { float3 p = i.wPos; float4 color = i.color; color.xyz += dot(i.normal, _WorldSpaceLightPos0.xyz) / 2; return saturate(color); }
  uint ADraw_o_i(v2f o) { return roundu(o.ti.x); }
  float4 frag_ADraw_Text(Texture2D t, RWStructuredBuffer<uint> _text, RWStructuredBuffer<ADraw_FontInfo> ADraw_fontInfos, float ADraw_fontSize, uint quadType, float4 backColor, uint2 textIs, v2f i)
  {
    float uv_x = 0;
    ...
    for (uint textI = textIs.x; textI < textIs.y; textI++)
    {
      ...
      float dx = f.advance / g.ADraw_fontSize;
      float2 mn = float2(f.minX, f.minY) / g.ADraw_fontSize, mx = float2(f.maxX, f.maxY) / g.ADraw_fontSize, range = mx - mn;
      if (quadType == ADraw_Text_QuadType_Arrow) { mn.y -= 0.5f; mx.y -= 0.5f; }
      float2 uv = (i.uv - uv_x * f10 + f1_ * mn - float2(f.bearing / g.ADraw_fontSize, 0.25f)) / range;
  ...
  v2f ADraw_o_i(uint i, v2f o) { o.ti.x = i; return o; }
  v2f ADraw_o_drawType(uint drawType, v2f o) { o.ti.z = drawType; return o; }
  v2f ADraw_o_color(float4 color, v2f o) { o.color = color; return o; }
  v2f ADraw_o_normal(float3 normal, v2f o) { o.normal = normal; return o; }
  v2f ADraw_o_uv(float2 uv, v2f o) { o.uv = uv; return o; }
  v2f ADraw_o_wPos(float3 wPos, v2f o) { o.wPos = wPos; return o; }
  float4 palette(float v) { return paletteColor(_PaletteTex, v); }
  float4 GetColor(Particle p) { return palette(length(p.velocity) / 2); }
  v2f ADraw_o_p0(float3 p0, v2f o) { o.p0 = p0; return o; }
  v2f ADraw_o_p1(float3 p1, v2f o) { o.p1 = p1; return o; }
  v2f ADraw_o_r(float r, v2f o) { o.ti.w = r; return o; }
  v2f ADraw_o_pos(float4 pos, v2f o) { o.pos = pos; return o; }
  v2f vert_ADraw_Point(float3 p, float4 color, uint i, v2f o) { return ADraw_o_i(i, ADraw_o_drawType(ADraw_Draw_Point, ADraw_o_color(color, ADraw_o_pos(UnityObjectToClipPos(float4(p, 1)), o)))); }
  v2f vert_Points(uint i, uint j, v2f o) { Particle p = GetParticle(i); return vert_ADraw_Point(p.position, GetColor(p), i, o); }
  v2f ADraw_o_pos_PV(float3 p, float4 q, v2f o) { return ADraw_o_pos(mul(UNITY_MATRIX_P, mul(UNITY_MATRIX_V, float4(p, 1)) + q), o); }
  v2f ADraw_o_pos_c(float4 c, v2f o) { return ADraw_o_pos(UnityObjectToClipPos(c), o); }
  v2f ADraw_o_pos_c(float3 c, v2f o) { return ADraw_o_pos(UnityObjectToClipPos(c), o); }
  float ADraw_o_r(v2f o) { return o.ti.w; }
  float4 frag_ADraw_Line(v2f i) { float3 p0 = i.p0, p1 = i.p1; float lineRadius = ADraw_o_r(i); float2 uv = i.uv; float r = dot(uv, uv), r2 = lineRadius * lineRadius; float4 color = i.color; float3 p10 = p1 - p0; float lp10 = length(p10); if (uv.x < 0) r /= r2; else if (uv.x > lp10) { uv.x -= lp10; r = dot(uv, uv) / r2; } else { uv.x = 0; r = dot(uv, uv) / r2; } if (r > 1.0f || color.a == 0) return f0000; float3 n = new float3(uv, r - 1), _LightDir = new float3(0.321f, 0.766f, -0.557f); float lightAmp = max(0.0f, dot(n, _LightDir)); float4 diffuse_Light = (lightAmp + UNITY_LIGHTMODEL_AMBIENT) * color; float spec = max(0, (lightAmp - 0.95f) / 0.05f); color = lerp(diffuse_Light, f1111, spec / 4); color.a = 1; return color; }
  float4 frag_ADraw_Arrow(v2f i) { float3 p0 = i.p0, p1 = i.p1; float lineRadius = ADraw_o_r(i); float2 uv = i.uv; float r = dot(uv, uv), r2 = lineRadius * lineRadius; float4 color = i.color; float3 p10 = p1 - p0; float lp10 = length(p10); if (uv.x < 0) r /= r2; else if (uv.x > lp10 - lineRadius * 3 && abs(uv.y) > lineRadius) { uv.x -= lp10; uv = rotate_sc(uv, -sign(uv.y) * 0.5f, 0.866025404f); uv.x = 0; r = dot(uv, uv) / r2; } else if (uv.x > lp10) { uv.x -= lp10; r = dot(uv, uv) / r2; } else { uv.x = 0; r = dot(uv, uv) / r2; } if (r > 1.0f || color.a == 0) return f0000; float3 n = new float3(uv, r - 1), _LightDir = new float3(0.321f, 0.766f, -0.557f); float lightAmp = max(0.0f, dot(n, _LightDir)); float4 diffuse_Light = (lightAmp + UNITY_LIGHTMODEL_AMBIENT) * color; float spec = max(0, (lightAmp - 0.95f) / 0.05f); color = lerp(diffuse_Light, f1111, spec / 4); color.a = 1; return color; }
  float4 frag_ADraw_LineSegment(v2f i) { float3 p0 = i.p0, p1 = i.p1; float lineRadius = ADraw_o_r(i); float2 uv = i.uv; float r = dot(uv, uv), r2 = lineRadius * lineRadius; float4 color = i.color; float3 p10 = p1 - p0; float lp10 = length(p10); uv.x = 0; r = dot(uv, uv) / r2; if (r > 1.0f || color.a == 0) return f0000; float3 n = new float3(uv, r - 1), _LightDir = new float3(0.321f, 0.766f, -0.557f); float lightAmp = max(0.0f, dot(n, _LightDir)); float4 diffuse_Light = (lightAmp + UNITY_LIGHTMODEL_AMBIENT) * color; float spec = max(0, (lightAmp - 0.95f) / 0.05f); color = lerp(diffuse_Light, f1111, spec / 4); color.a = 1; return color; }
  uint ADraw_SignalSmpN(uint chI) { return 1024; }
  float ADraw_SignalThickness(uint chI, uint smpI) { return 0.004f; }
  float ADraw_SignalSmpV(uint chI, uint smpI) { return 0; }
  float4 ADraw_SignalColor(uint chI, uint smpI) { return YELLOW; }
  float ADraw_SignalFillCrest(uint chI, uint smpI) { return 1; }
  float4 ADraw_SignalMarker(uint chI, float smpI) { return f0000; }
  float4 ADraw_SignalBackColor(uint chI, uint smpI) { return float4(1, 1, 1, 0.2f); }
  float4 frag_ADraw_Signal(v2f i)
  {
  ...
  float4 frag_ADraw_GS(v2f i, float4 color)
  {
    switch (ADraw_o_drawType(i))
    {
      ...
      case ADraw_Draw_Text3D:
        ...
        color = frag_ADraw_Text(ADraw_fontTexture, ADraw_tab_delimeted_text, ADraw_fontInfos, g.ADraw_fontSize, t.quadType, t.backColor, ADraw_Get_text_indexes(t.textI), i);
        break;
    }
    return color;
  }
  float4 frag_GS(v2f i, float4 color) { return frag_ADraw_GS(i, color); }
  ...
  float4 ADraw_Sphere_quadPoint(float r, uint j) { return r * float4(2 * ADraw_JQuadf(j) - 1, 0, 0); }
  v2f vert_ADraw_Sphere(float3 p, float r, float4 color, uint i, uint j, v2f o) { float4 q = ADraw_Sphere_quadPoint(r, j); return ADraw_o_i(i, ADraw_o_drawType(ADraw_Draw_Sphere, ADraw_o_color(color, ADraw_o_normal(-f001, ADraw_o_uv(q.xy / r, ADraw_o_pos_PV(p, q, ADraw_o_wPos(p, o))))))); }
  v2f vert_Spheres(uint i, uint j, v2f o) { Particle p = GetParticle(i); return vert_ADraw_Sphere(p.position, 0.01f, GetColor(p), i, j, o); }
  ...
  float4 ADraw_LineArrow_p4(float dpf, float3 p0, float3 p1, float r, uint j) { return ADraw_LineArrow_p4(dpf, p0, p1, _WorldSpaceCameraPos, r, j); }
  v2f vert_ADraw_Line(float3 p0, float3 p1, float r, float4 color, uint i, uint j, v2f o) { return ADraw_o_i(i, ADraw_o_p0(p0, ADraw_o_p1(p1, ADraw_o_r(r, ADraw_o_drawType(ADraw_Draw_Line, ADraw_o_color(color, ADraw_o_uv(ADraw_Line_uv(p0, p1, r, j), ADraw_o_pos_c(ADraw_LineArrow_p4(1, p0, p1, r, j), o)))))))); }
  v2f vert_Lines(uint i, uint j, v2f o) { Particle p = GetParticle(i); float3 d = GetDirection(p); return vert_ADraw_Line(p.position - d, p.position + d, length(d) / 10, GetColor(p), i, j, o); ; }
  ...
  v2f vert_ADraw_LineArrow(float dpf, float3 p0, float3 p1, float r, float4 color, uint i, uint j, v2f o) { return ADraw_o_i(i, ADraw_o_p0(p0, ADraw_o_p1(p1, ADraw_o_r(r, ADraw_o_drawType(dpf == 1 ? ADraw_Draw_Line : ADraw_Draw_Arrow, ADraw_o_color(color, ADraw_o_uv(ADraw_LineArrow_uv(dpf, p0, p1, r, j), ADraw_o_pos_c(ADraw_LineArrow_p4(dpf, p0, p1, r, j), o)))))))); }
  ...
  v2f vert_Arrows(uint i, uint j, v2f o) { Particle p = GetParticle(i); float3 d = GetDirection(p); return vert_ADraw_Arrow(p.position - d, p.position + d, length(d) / 10, GetColor(p), i, j, o); }
  v2f base_vert_ADraw_Text(ADraw_TextInfo t, uint i, uint j, v2f o)
  {
  ...
  v2f vert_ADraw_Text(ADraw_TextInfo t, uint i, uint j, v2f o)
  {
    ...
    t.p = p.position;
    if (g.textQuadType == ADraw_Text_QuadType_Arrow) { float3 d = GetDirection(p); t.p0 = p.position - d; t.p1 = p.position + d; }
    t.quadType = (uint)g.textQuadType;
    t.color = GetColor(p);
    t.height = g.textHeight;
    return base_vert_ADraw_Text(t, i, j, o);
  }
