Shader "gs/gsSwarm"
{
  ...
  struct GSwarm
  {
    ...
    uint randomArrayI, particleN, particleShape, textQuadType, particleDistribution, BDraw_AppendBuff_IndexN, BDraw_AppendBuff_BitN, BDraw_AppendBuff_N, BDraw_AppendBuff_BitN1, BDraw_AppendBuff_BitN2, BDraw_omitText, BDraw_includeUnicode, BDraw_fontInfoN, BDraw_textN, BDraw_textCharN, BDraw_boxEdgeN, Rand_N, Rand_I, Rand_J;
  ...
  v2f vert_Points(uint i, uint j, v2f o) { Particle p = GetParticle(i); return vert_BDraw_Point(p.position, GetColor(p), i, o); }
  ...
  v2f vert_Spheres(uint i, uint j, v2f o) { Particle p = GetParticle(i); return vert_BDraw_Sphere(p.position, 0.01f, GetColor(p), i, j, o); }
  ...
  v2f vert_Arrows(uint i, uint j, v2f o) { Particle p = GetParticle(i); float3 d = GetDirection(p); return vert_BDraw_Arrow(p.position - d, p.position + d, length(d) / 10, GetColor(p), i, j, o); }
  ...
  v2f vert_BDraw_Text(BDraw_TextInfo t, uint i, uint j, v2f o)
  {
    ...
    t.p = p.position;
    if (g.textQuadType == BDraw_Text_QuadType_Arrow) { float3 d = GetDirection(p); t.p0 = p.position - d; t.p1 = p.position + d; }
