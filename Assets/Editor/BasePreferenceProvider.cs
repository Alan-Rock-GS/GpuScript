...
public abstract class BasePreferenceProvider : IPreferenceProvider
{
	...
	public object ValueField(string valueName, object value)
	{
		...
		if (bytes != null)
		{
			...
			if (EditorGUI.EndChangeCheck()) return Encoding.UTF8.GetBytes(newString);
		}
		else if (value is int)
		{
			...
			if (EditorGUI.EndChangeCheck()) return newInt;
		}
		else if (value is float)
		{
			...
			if (EditorGUI.EndChangeCheck()) return newFloat;
		}
		else EditorGUILayout.LabelField(NicifyValueName(valueName), string.Format("Unhandled Type {0}", value.GetType()));
		return value;
	}
	protected virtual string NicifyValueName(string keyValueName) => keyValueName;
}
