...
public class WindowsPreferenceProvider : BasePreferenceProvider
{
	...
	public override void SetKeyValue(string valueName, object newValue)
	{
		...
		{
			if (key == null) throw new KeyNotFoundException(string.Format("Failed to open sub key {0}.", kUnityRootSubKey));
	...
	public override void FetchKeyValues(IDictionary<string, object> prefsLookup)
	{
		using (RegistryKey key = Registry.CurrentUser.OpenSubKey(kUnityRootSubKey, false))
		{
			if (key == null) throw new KeyNotFoundException(string.Format("Failed to open sub key {0}.", kUnityRootSubKey));
			prefsLookup.Clear();
			foreach (string keyValueName in key.GetValueNames()) { var value = key.GetValue(keyValueName); prefsLookup.Add(keyValueName, value); }
		}
	}
	protected override string NicifyValueName(string keyValueName) => keyValueName.Split(new[] { "_h" }, StringSplitOptions.None).First();
}