...
public class GS_Window : EditorWindow
{
	...
	public void Lib_Paste_clicked(ClickEvent evt = null)
	{
		...
		if (s.Contains("\n"))
		{
			...
			items.For(item => s = item.Item1 switch { "Email" => email = item.Item2, "Library" => lib = item.Item2, "Expires" => expires = item.Item2, "Key" => key = item.Item2, _ => "" });
		...
		if (text.Contains(","))
		{
			...
			if (items.Length == 4)
			{
				...
				foreach (var f in _GS_files)
				{
					...
					if (s != original_s) f.WriteAllText(s);
				}
			}
		}
	}
	...
	static void EditorGlobalKeyPress() { eventType0 = eventType; eventType = Event.current.type; keyCode0 = keyCode; keyCode = Event.current.keyCode; }
	...
	void SelectionChanged()
	{
		...
		if (o != null && o.name.StartsWith("gs") && gsClass_name != null && o.name.After("gs") == sceneName) gsClass_name_val = gsClass_name.value = o.name;
	}
	...
	public class ProjectData
	{
		...
		void Get_gStruct()
		{
			...
			foreach (var fld in _GS_fields)
			{
				if (fld.FieldType.IsNotAny(typeof(TreeGroup), typeof(TreeGroupEnd), typeof(string), typeof(strings), typeof(Texture2D)) && !fld.FieldType.IsArray)
				{
					...
					if (typeName.StartsWith("gs")) lib_flds.Add(fld);
					else { StackFields(gStruct, typeName, fld.Name); gpu_flds.Add(fld); }
				}
			}
			gStruct.Add("\n  };");
		}
		...
		void Get_USS_Text(StrBldr text, int height, params string[] names)
		{
			...
			foreach (var name in names) UI_VisualElement.Get_USS_Text(text, name, fontSize: fontSize, alignText: alignText, height: height, padding: 0, margin: 0, border: 0);
		}
 ...
 $"    <Style src=\"project://database/Assets/GS/Resources/UI/GpuScript/gs_USS.uss?fileID=7433441132597879392&amp;guid={new_uxml_guid()}&amp;type=3#{gsName}_USS\" />",
	...
	"</ui:UXML>");
		...
		{
			public method_data(string Name, string methodName) { this.methodName = methodName; name = $"g{Name}_{methodName}_GS"; args = "uint3 id"; return_type = "void"; code = ""; }
	...
	"");
		...
		public void Get_compute_or_material_shader()
		{
			...
			for (int i = render_methods.Length - 1; i >= 0; i--)
			{
				...
				if (m != null) { if (!m.renderQuads) rMethods.Add(m); else rMethods.Insert(0, m); }
			}
			...
			compute_or_material_shader.Add(" return LIN; }",
				"\n  public virtual uint3 onRenderObject_LIN_Quads(uint i) { uint3 LIN = u000;");
			foreach (var m in render_methods) if (m != null && m.renderQuads) { compute_or_material_shader.Add(" uint index = 0;"); break; }
			...
			compute_or_material_shader.Add(" return LIN; }",
				"\n  public virtual uint3 onRenderObject_LIN_Points(uint i) { uint3 LIN = u000;");
			foreach (var m in render_methods) if (m != null && !m.renderQuads) { compute_or_material_shader.Add(" uint index = 0;"); break; }
			foreach (var m in render_methods) if (m != null && !m.renderQuads) compute_or_material_shader.Add($" onRenderObject_LIN({m.showIf}{m.size}, ref i, ref index, ref LIN);");
			compute_or_material_shader.Add(" return LIN; }");
		}
			...
			if (generateComputeShader)
			{
				if (kernel_methods != null)
				{
					...
					foreach (var k in kernel_methods)
					{
						...
						if (uintN.Contains(".") && uintN.After(".").DoesNotStartWithAny("x", "y", "z", "w"))
							uintN = uintN.Replace(".", "_");
...
$"\n    {m_name}_To_UI();",
 "");
	 ...
	 "\n  public virtual void Start() { Build_UI(); Load_UI(); Start0_GS(); InitBuffers(); Start1_GS(); }", s_start0_GS, s_start1_GS,
			...
			"\n  		do",
			"\n  		{",
			"\n  			double f = uint2_to_double(uint2(double_uints[I], double_uints[I + 1]));",
			"\n  			uint2 F = double_to_uint2(f), F2 = double_to_uint2(v + f);",
			"\n  			Ok = Ok || Is(F == F2) || bool2(Ok.x || InterlockedCompareExchange(double_uints, I, F.x, F2.x) == F.x, Ok.y || InterlockedCompareExchange(double_uints, I + 1, F.y, F2.y) == F.y);",
			"\n  		} while (!all(Ok));",
			"\n  	 }",
			"\n	 }",
			"\n	 public void InterlockedMul_Double(RWStructuredBuffer<uint> double_uints, uint I, double v)",
			"\n	 {",
			"\n	  if (v != 1)",
			"\n	  {",
			"\n	  	bool2 Ok = b00;",
			...
			"\n	   }",
			"\n  }",
			"\n  public static void InterlockedMin_Double(RWStructuredBuffer<uint> double_uints, uint I, double v)",
			"\n  {",
			"\n  	bool2 Ok = b00;",
			"\n  	I *= 2;",
			"\n  	do",
			"\n  	{",
			"\n  		double f = uint2_to_double(uint2(double_uints[I], double_uints[I + 1]));",
			"\n  		uint2 F = double_to_uint2(f), F2 = double_to_uint2(min(v, f));",
			"\n  		Ok = Ok || Is(F == F2) || bool2(Ok.x || InterlockedCompareExchange(double_uints, I, F.x, F2.x) == F.x, Ok.y || InterlockedCompareExchange(double_uints, I + 1, F.y, F2.y) == F.y);",
			"\n  	} while (!all(Ok));",
			"\n  }",
			"\n  public static void InterlockedMax_Double(RWStructuredBuffer<uint> double_uints, uint I, double v)",
			"\n  {",
			"\n  	bool2 Ok = b00;",
			"\n  	I *= 2;",
			"\n  	do",
			"\n  	{",
			"\n  		double f = uint2_to_double(uint2(double_uints[I], double_uints[I + 1]));",
			"\n  		uint2 F = double_to_uint2(f), F2 = double_to_uint2(max(v, f));",
			"\n  		Ok = Ok || Is(F == F2) || bool2(Ok.x || InterlockedCompareExchange(double_uints, I, F.x, F2.x) == F.x, Ok.y || InterlockedCompareExchange(double_uints, I + 1, F.y, F2.y) == F.y);",
			"\n  	} while (!all(Ok));",
			"\n  }",
			...
			bool wrote_cs_File = _cs_filename.WriteAllText_IfChanged(_cs_str.ReplaceTabsWithSpaces());
		...
		public string Scenes_in_Build
		{
			get
			{
				...
				bool firstTime = true;
				for (int i = 0; i < scenes.Length; i++) { var scene = scenes[i]; if (scene.enabled) { s.Add($"{(firstTime ? "" : ", ")}\"{scene.path}\""); firstTime = false; } }
				s.Add(" };");
				return s;
			}
		}
		...
		"\n}");
			...
			if (!gs.material)
			{
				...
				if (shader)
				{
					...
					if (gs.material) { Component script = GameObject.Find(gsName).GetComponent(gsName); script.GetType().GetField("material")?.SetValue(script, gs.material); }
				}
			}
		...
		public void Build_Precompiled_Lib(string cs_Code, string _cs_Code, string _GS_Code, string path)
		{
			...
			foreach (var mem in _gs_members)
				if (mem.AttGS() != null && mem.IsArray() && mem.GetMemberType().GetElementType().IsClass && classList.DoesNotContain(mem.Name)) classList.Add(mem.Name);
	...
	void keep(string path) { string p = omitPath(path), n = p.After("/gs").Before("/"), q = $"{p}{n}.unity"; if (q.Exists()) { p.CopyDirAll(path); p.DeleteDirectory(); } }
	...
	{
		foreach (var a in GS_Assemblies) { Type t = Type.GetType($"{name}, {a}_Assembly, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null"); if (t != null) return t; }
		return null;
	}
	...
	static Component FindOrCreate_Script(GameObject o, string _GS)
	{
		...
		if (!GS_script) { Type componentType = GetComponentTypeByName(_GS); if (componentType != null) { GS_script = o.GetComponent(componentType); GS_script ??= o.AddComponent(componentType); } }
		return GS_script;
	}
	...
	public void Build_GS_Exe()
	{
		...
		if (apis.Length == 1) { var apis2 = new UnityEngine.Rendering.GraphicsDeviceType[2]; apis2[1] = apis[0]; apis2[0] = UnityEngine.Rendering.GraphicsDeviceType.Direct3D12; apis = apis2; }
