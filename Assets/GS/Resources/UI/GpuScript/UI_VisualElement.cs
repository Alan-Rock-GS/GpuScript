...
{
  //[UxmlElement]
  public class UI_VisualElement : VisualElement
  {
    ...
    protected void SetPropertyValue(float val) { property?.SetValue(gs, iconvert(val) * convert(siUnit, GetUnitConversion(siUnit))); gs?.OnValueChanged(); }
    protected void SetPropertyValue(float2 val) { property?.SetValue(gs, iconvert(val) * convert(siUnit, GetUnitConversion(siUnit))); gs?.OnValueChanged(); }
    protected void SetPropertyValue(float3 val) { property?.SetValue(gs, iconvert(val) * convert(siUnit, GetUnitConversion(siUnit))); gs?.OnValueChanged(); }
    protected void SetPropertyValue(float4 val) { property?.SetValue(gs, iconvert(val) * convert(siUnit, GetUnitConversion(siUnit))); gs?.OnValueChanged(); }
    ...
    public virtual bool Init(GS gs, params GS[] gss)
    {
      ...
      if (fld != null)
      {
        ...
        if (treeGroup_parent != null)
        {
          ...
          if (this is UI_TreeGroup)
          {
            var t = this as UI_TreeGroup;
            t.Build(t.label, t.description, treeGroup_parent_name, t.toggle.value, treeGroup_parent.level + 1);
          }
        }
        return true;
      }
    ...
    public static void UXML(UI_Element e, AttGS att, string name, string label, string typeName)
    {
      ...
      if (label.IsNotEmpty()) e.uxml.Add(" UI_Label=\"", label, "\"");
      if (att.Description.IsNotEmpty()) e.uxml.Add($" UI_Description=\"{att.Description}\"");
      if (att.readOnly) e.uxml.Add($" UI_isReadonly=\"true\"");
      if (e.treeGroup?.name.IsNotEmpty() ?? false)
        e.uxml.Add($" UI_TreeGroup_Parent=\"{e.treeGroup.name}\"");
    ...
    public void Build(string Label, string Description, bool isReadOnly, string treeGroup_parent)
    {
      label = Label; description = Description; this.isReadOnly = isReadOnly; treeGroup_parent_name = treeGroup_parent;
    }
    ...
    public new class UxmlTraits : VisualElement.UxmlTraits
    {
      ...
      public UxmlStringAttributeDescription m_Label = new UxmlStringAttributeDescription { name = "UI_Label", defaultValue = "" };
      public UxmlStringAttributeDescription m_Description = new UxmlStringAttributeDescription { name = "UI_Description", defaultValue = "" };
      public UxmlStringAttributeDescription m_Format = new UxmlStringAttributeDescription { name = "UI_Format", defaultValue = "" };
      public UxmlBoolAttributeDescription m_isReadOnly = new UxmlBoolAttributeDescription { name = "UI_isReadonly", defaultValue = false };
      public UxmlBoolAttributeDescription m_isPow2Slider = new UxmlBoolAttributeDescription { name = "UI_isPow2Slider", defaultValue = false };
    ...
    [HideInInspector] public usUnit usUnit;
    [HideInInspector] public siUnit siUnit;
  ...
  public class TreeGroupEnd
  {
    public static void UXML_UI_Element(UI_Element e) { e.treeGroup = e.treeGroup?.treeGroup_parent; e.level--; e.uxml_level--; }
  }
