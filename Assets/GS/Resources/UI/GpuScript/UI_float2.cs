...
namespace GpuScript
{
  ...
  {
    public float2 Slider_Pow_Val(float2 v) => clamp(round(is_Pow2_Slider ? (pow10(abs(v)) - 1) / 0.999f : v, Nearest), range_Min, range_Max);
    public float2 Slider_Log_Val(float2 v) => is_Pow2_Slider ? log10(abs(clamp(round(v, Nearest), range_Min, range_Max)) * 0.999f + 1) : v;
      ...
      StrBldr showIfs, StrBldr onValueChanged, AttGS attGS, string typeStr, string name)
    {
      ...
      if (si == "Null") si = "_si";
      lateUpdate.Add($"\n    if (UI_{name}.Changed || any({name} != UI_{name}.{si})) {name} = UI_{name}.{si};");
    ...
    float2 _v0 = default;
    public float2 _si; public float2 si { get => _si; set => v = convert(_si = value); }
    public float2 v
    {
      get => textField != null ? _si = siUnits ? textField.value.To_float2() : iconvert(textField.value.To_float2()) : _si;
      set
      {
    ...
    float2 _range_Min; public float2 range_Min { get => _range_Min; set { _range_Min = value; if (sliders[0] != null) { var v = Slider_Log_Val(value); for (int i = 0; i < sliders.Length; i++) sliders[i].lowValue = v[i]; } } }
    float2 _range_Max; public float2 range_Max { get => _range_Max; set { _range_Max = value; if (sliders[0] != null) { var v = Slider_Log_Val(value); for (int i = 0; i < sliders.Length; i++) sliders[i].highValue = v[i]; } } }
    ...
    public override void OnTextFieldChanged(TextField o) { float2 val = o.value.To_float2(); SliderV = val; SetPropertyValue(val); }
      ...
      string siFormat, string usFormat, bool isReadOnly, bool isGrid, bool isPow2Slider, bool isPow10, bool isPow2, float nearest, string treeGroup_parent)
    {
      ...
      range_Min = rangeMin.To_float2(); range_Max = rangeMax.To_float2(); SliderV = val.To_float2();
    }
