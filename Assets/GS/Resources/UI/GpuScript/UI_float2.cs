...
namespace GpuScript
{
	...
	{
		public float2 Slider_Pow_Val(float2 v) => clamp(round(is_Pow2_Slider ? (pow10(abs(v)) - 1) / 0.999f : v, GetNearest(v)), range_Min, range_Max);
		public float2 Slider_Log_Val(float2 v) => is_Pow2_Slider ? log10(abs(clamp(round(v, GetNearest(v)), range_Min, range_Max)) * 0.999f + 1) : v;
