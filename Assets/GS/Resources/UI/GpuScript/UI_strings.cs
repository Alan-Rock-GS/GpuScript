...
namespace GpuScript
{
  ...
  {
    [UxmlAttribute] public string choices { get => dropdownField.choices.Join("|"); set => dropdownField.choices = value.Split("|").ToList(); }
    ...
    public override bool Init(GS gs, params GS[] gss)
    {
      ...
      {
        dropdownField.choices = choices?.Split("|").ToList() ?? "A|B|C"?.Split("|").ToList();
   ...
   public static new void UXML(UI_Element e, AttGS att, string name, string label, string typeName)
    {
      ...
      if (att.Vals != null) e.uxml.Add(" choices=\"", att.Vals.ToString(), "\"");
      if (att.Val != null) e.uxml.Add(" val=\"", att.Val.ToString(), "\"");
    }
