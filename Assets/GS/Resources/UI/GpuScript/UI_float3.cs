...
{
	[UxmlElement]
	public partial class UI_float3 : UI_Slider_base
	{
		public override bool Init(GS gs, params GS[] gss)
		{
			...
			SliderV = v = val.To_float3(); rangeMin = RangeMin.To_float3(); rangeMax = RangeMax.To_float3(); ;
		...
		float3 _rangeMin; public float3 rangeMin { get => _rangeMin; set { _rangeMin = value; if (sliders[0] != null) { var v = Slider_Log_Val(value); for (int i = 0; i < sliders.Length; i++) sliders[i].lowValue = v[i]; } } }
		float3 _rangeMax; public float3 rangeMax { get => _rangeMax; set { _rangeMax = value; if (sliders[0] != null) { var v = Slider_Log_Val(value); for (int i = 0; i < sliders.Length; i++) sliders[i].highValue = v[i]; } } }
		...
		public float3 deg_per_sec { get => si * convert(Unit, Unit.deg_per_sec); set => si = value / convert(Unit, Unit.deg_per_sec); }
		public float3 rad_per_sec { get => si * convert(Unit, Unit.rad_per_sec); set => si = value / convert(Unit, Unit.rad_per_sec); }
		public float3 rpm { get => si * convert(Unit, Unit.rpm); set => si = value / convert(Unit, Unit.rpm); }
		public float3 rps { get => si * convert(Unit, Unit.rps); set => si = value / convert(Unit, Unit.rps); }
