...
namespace GpuScript
{
  ...
  {
    public float3 Slider_Pow_Val(float3 v) => clamp(is_Pow2_Slider ? sign(v) * (pow10(abs(v)) - 1) / 0.999f : v, range_Min, range_Max); 
    public float3 Slider_Log_Val(float3 v) => is_Pow2_Slider ? sign(v) * log10(abs(v) * 0.999f + 1) : v; 
    ...
    float3 _v0 = default;
    public float3 _si; public float3 si { get => _si; set => v = convert(_si = value); }
    public float3 v
    {
      get => textField != null ? _si = siUnits ? textField.value.To_float3() : iconvert(textField.value.To_float3()) : _si;
      set
      {
    ...
    public override void OnValueChanged(ChangeEvent<float> evt) { if (evt.currentTarget is Slider && textField != null) { float3 val = SliderV; textField.value = val.ToString(format); SetPropertyValue(val); } }
    public override void OnTextFieldChanged(TextField o) { float3 val = o.value.To_float3(); SliderV = val; SetPropertyValue(val); }
