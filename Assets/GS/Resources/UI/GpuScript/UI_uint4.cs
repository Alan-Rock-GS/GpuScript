...
namespace GpuScript
{
  ...
  public partial class UI_uint4 : UI_Slider_base
  {
    public override bool Init(GS gs, params GS[] gss)
    {
      ...
      SliderV = v = _val.To_uint4(); rangeMin = RangeMin.To_uint4(); rangeMax = RangeMax.To_uint4();
    ...
    public float4 Slider_Log_Val(uint4 v) => isPow2Slider ? log10(clamp(roundu(v, GetNearest((float4)v)), rangeMin, rangeMax) * 0.999f + 1) : (float4)v;
    ...
    uint4 _v = default, _val = default;
    public uint4 v
    {
      get => textField != null ? _val = textField.value.To_uint4() : _val;
      set
      {
        _val = isPow10 ? roundu(pow10(round(log10((float4)value)))) : isPow2 ? roundu(pow2(round(log2(value)))) : value;
