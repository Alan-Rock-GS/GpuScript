...
namespace GpuScript
{
  ...
  {
    public uint3 Slider_Pow_Val(float3 v) => clamp(roundu(is_Pow2_Slider ? (pow10(abs(v)) - 1) / 0.999f : v, GetNearest(v)), range_Min, range_Max);
    public float3 Slider_Log_Val(uint3 v) => is_Pow2_Slider ? log10(clamp(roundu(v, GetNearest(v)), range_Min, range_Max) * 0.999f + 1) : (float3)v;
