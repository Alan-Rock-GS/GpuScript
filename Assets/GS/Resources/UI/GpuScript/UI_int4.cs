...
{
  [UxmlElement]
  public partial class UI_int4 : UI_Slider_base
  {
    ...
    public int4 Slider_Pow_Val(float4 v) => clamp(roundi(isPow2Slider ? (pow10(abs(v)) - 1) / 0.999f : v, GetNearest(v)), rangeMin, rangeMax);
    public float4 Slider_Log_Val(int4 v) => isPow2Slider ? log10(abs(clamp(roundi(v, GetNearest((float4)v)), rangeMin, rangeMax)) * 0.999f + 1) : v;
    ...
    public int4 v
    {
      ...
      set { 
        _val = isPow10 ? roundi(pow10(round(log10((float4)value)))) : isPow2 ? roundi(pow2(round(log2(value)))) : value;
    ...
    int4 _rangeMin; public int4 rangeMin { get => _rangeMin; set { _rangeMin = value; if (sliders[0] != null) { var v = Slider_Log_Val(value); for (int i = 0; i < sliders.Length; i++) sliders[i].lowValue = v[i]; } } }
    int4 _rangeMax; public int4 rangeMax { get => _rangeMax; set { _rangeMax = value; if (sliders[0] != null) { var v = Slider_Log_Val(value); for (int i = 0; i < sliders.Length; i++) sliders[i].highValue = v[i]; } } }
