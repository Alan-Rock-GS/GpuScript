...
namespace GpuScript
{
  ...
  public class UI_int4 : UI_Slider_base
  {
      ...
      bool isPow10, bool isPow2, float nearest, bool nearestDigit, string treeGroup_parent)
    {
      ...
      this.rangeMin = rangeMin.To_int4(); this.rangeMax = rangeMax.To_int4(); SliderV = val.To_int4();
      if (headerLabel != null) headerLabel.HideIf(label.IsEmpty() || isGrid);
    }
    ...
    public float4 Slider_Log_Val(int4 v) => isPow2Slider ? log10(abs(clamp(roundi(v, GetNearest(v)), rangeMin, rangeMax)) * 0.999f + 1) : v;
