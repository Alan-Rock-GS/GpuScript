...
namespace GpuScript
{
  ...
  {
    public int4 Slider_Pow_Val(float4 v) => clamp(roundi(is_Pow2_Slider ? (pow10(abs(v)) - 1) / 0.999f : v), range_Min, range_Max); 
    public float4 Slider_Log_Val(int4 v) => is_Pow2_Slider ? log10(abs(v) * 0.999f + 1) : (float4)v; 
    ...
    {
      if (hasRange) SliderV = o.value.To_int4(); else { property.SetValue(gs, textField.value.To_int4()); gs.OnValueChanged(); }
    }
