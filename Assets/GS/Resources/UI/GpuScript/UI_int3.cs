...
namespace GpuScript
{
  ...
  {
    public int3 Slider_Pow_Val(float3 v) => clamp(roundi(is_Pow2_Slider ? (pow10(abs(v)) - 1) / 0.999f : v), range_Min, range_Max); 
    public float3 Slider_Log_Val(int3 v) => is_Pow2_Slider ? log10(abs(v) * 0.999f + 1) : (float3)v; 
