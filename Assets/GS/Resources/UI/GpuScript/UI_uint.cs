...
namespace GpuScript
{
  public class UI_uint : UI_Slider_base
  {
    ...
    public override void OnValueChanged(ChangeEvent<float> evt) { if (evt.currentTarget is Slider && textField != null) { var val = SliderV; textField.value = val.ToString(format); property?.SetValue(gs, val); gs?.OnValueChanged(); } }
