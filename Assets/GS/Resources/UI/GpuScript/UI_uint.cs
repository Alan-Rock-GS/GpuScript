...
namespace GpuScript
{
  ...
  {
    public uint Slider_Pow_Val(float v) => clamp(roundu(is_Pow2_Slider ? (pow10(abs(v)) - 1) / 0.999f : v, GetNearest(flooru(v))), range_Min, range_Max);
    public float Slider_Log_Val(uint v) => is_Pow2_Slider ? log10(abs(clamp(round(v, GetNearest(v)), range_Min, range_Max)) * 0.999f + 1) : v;
