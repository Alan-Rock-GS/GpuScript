...
namespace GpuScript
{
	...
	public partial class UI_uint : UI_Slider_base
	{
		...
		public uint get_val(uint x)
		{
			x = isPow10 ? roundu(pow10(round(log10(x)))) : isPow2 ? roundu(pow2(round(log2(x)))) : x;
			if (Nearest > 0) x = roundu(x, Nearest); if (NearestDigit) x = GetNearestDigit(x);
			return x;
		}
		...
		//	}
		//}
		//public override void OnTextFieldChanged(TextField o) { var x = get_val(o.value.To_uint()); if (isGrid || x != v) { if (hasRange) SliderV = x; if (!GS.isGridVScroll && !GS.isGridBuilding) SetPropertyValue(x); } }
		public override void OnTextFieldChanged(TextField o)
		{
			var x = get_val(o.value.To_uint());
			if (isGrid || x != v)
		...
		//	if (isGrid)
		//	{
		//		uint x = get_val(o.value.To_uint());
		//		if (hasRange)
		//			SliderV = x;
		//		SetPropertyValue(x);
		//	}
